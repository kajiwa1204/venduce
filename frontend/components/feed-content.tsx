...
function PostItem({ post, onLikeToggle }: PostItemProps) {
  ...
  return (
    <article className="w-full md:w-[50%] overflow-hidden border-b md:border-b-0 md:border border-border bg-card pb-4 md:rounded-xl mx-6 md:mx-0">
      {/* Header */}
      <div
        className="flex items-center justify-between p-3 cursor-pointer hover:bg-muted/50 transition-colors md:rounded-t-lg"
        onClick={handlePostClick}
      >
        <div className="flex items-center gap-3">
          <Avatar>
            <AvatarImage
              src={getImageUrl(post.user?.avatar_url ?? undefined)}
            />
            <AvatarFallback>{post.user?.username?.[0] ?? "?"}</AvatarFallback>
          </Avatar>
          <div className="leading-tight">
            <p className="font-semibold text-sm">
              {post.user?.username ?? "匿名ユーザー"}
            </p>
          </div>
        </div>
      </div>
            ...
            >
              {assets.map((asset, index) => {
                ...
                return (
                  <div
                    key={asset.id || index}
                    className="w-full h-full flex-shrink-0 snap-center relative group"
                  >
                    <img
                    ...
                    {linkedProduct && (
                      <div
                        className="absolute bottom-4 right-4 flex gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity"
                      ...
                      >
                        <Link
                          ...
                          className={cn(
                            ...
                            "gap-1.5 px-3 shadow-md bg-white/90 text-black hover:bg-white",
                          )}
                        >
      ...
      {/* Actions & Caption */}
      <div
        className="px-3 pt-3 cursor-pointer hover:bg-muted/50 transition-colors"
        onClick={handlePostClick}
      >
...
export function FeedContent() {
  ...
  return (
    <div className="space-y-4 md:space-y-6 flex flex-col items-center mt-6 mb-6 px-6">
      {posts.map((post) => (
      ...
      >
        {isLoadingMore && (
          <div className="text-sm text-muted-foreground">追加読み込み中...</div>
        )}
